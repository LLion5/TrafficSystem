...
import cn.zjgsu.lightsever.util.PortToData;
...
public class SerialView extends JFrame {
    ...
    Image icon = toolKit.getImage("res//computer.png");
    ...
    private void init() {
        ...
        receiveScroll.setHorizontalScrollBarPolicy( 
                JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);   //确定水平滚动条何时显示在滚动窗格上
    ...
    class SerialListener implements SerialPortEventListener {
        ...
        public void serialEvent(SerialPortEvent serialPortEvent) {
            ...
            case SerialPortEvent.DATA_AVAILABLE: // 1 串口存在可用数据
                try {
                	 SerialView.this.str.append(SerialTool.ReadCar(SerialView.this.getSerialPort()));
             			if(SerialView.str.length()>=1) {
             				SerialTool.Id.replace(0, SerialView.this.str.length(), "");
             				System.out.println(SerialView.str.length());
             				String time=df.format(LocalDateTime.ofInstant(Instant.ofEpochMilli(System.currentTimeMillis()),ZoneId.of("Asia/Shanghai")));
             				receiveArea.append(time+" ["+serialPort.getName().split("/")[3]+"] : "+ str+"\r\n");
                            receiveArea.setCaretPosition(receiveArea.getText().length());  
                            new PortToData(SerialView.this.str.toString());
                            SerialView.this.str.replace(0, SerialView.this.str.length(), "");
             			}
